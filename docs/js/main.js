...
(function () {
  ...
  async function fetchCharacters() {
    ...
    // Fetch live GitHub star counts (non-blocking, best-effort)
    const repos = [...new Set(CHARACTERS.map(c => c.github).filter(Boolean))];
    const starPromises = repos.map(repo =>
      fetch(`https://api.github.com/repos/${repo}`)
        .then(r => r.ok ? r.json() : null)
        .then(d => d ? { repo, stars: d.stargazers_count } : null)
        .catch(() => null)
    );
    try {
      const results = await Promise.all(starPromises);
      const starMap = {};
      results.forEach(r => { if (r) starMap[r.repo] = r.stars; });
      CHARACTERS.forEach(ch => {
        if (ch.github && starMap[ch.github] !== undefined) {
          ch.stars = starMap[ch.github];
        }
      });
    } catch (e) {
      ...
      chars.forEach(ch => {
        ...
        card.innerHTML = `
          ...
          <div class="mp-card-footer">
            ...
            <span class="mp-card-stars" title="${ch.stars} GitHub stars">${ch.stars > 0 ? `&#x2B50; ${ch.stars}` : '&#x1F195; New'}</span>
    ...
    body.innerHTML = `
      <div class="detail-grid">
        <div class="detail-avatar">
          ...
          ${officialHTML}
          <div class="detail-stars">${ch.stars > 0 ? `&#x2B50; ${ch.stars} stars` : '&#x1F195; New'}</div>
        </div>
